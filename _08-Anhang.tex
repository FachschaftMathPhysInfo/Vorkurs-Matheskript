...
\begin{phasetwo}[Rumprobieren] \label{rumprobieren}
...
\begin{phasethree}[Aufschreiben]\label{beweisaufschreiben}
...
\begin{description}[labelindent=0pt, leftmargin=0pt]
    ...
    \item[Modulo:] \index{modulo} Das Arbeiten mit Faktormengen. Der Begriff wird auch im übertragenen Sinne verwendet, wenn Objekte nicht vollständig, aber bis auf „vernachlässigbare“ Unterschiede voneinander unterschieden werden. Beispielsweise ist die Lösung der Gleichung $x^2=2$ „eindeutig bis auf (\glq modulo\grq) Vorzeichen“.
...
\begin{comment}
\chapter{Einige metamathematische Bemerkungen} \label{philosohpie}


Ich konnte der Versuchung nicht widerstehen, hier einige abschweifende persönliche Gedanken zu hinterlassen.

Mathematisches Arbeiten geschieht meiner Ansicht nach auf drei Ebenen. Deren erste beiden sind:
\begin{enumerate}[1.]
    \item Die sprachliche Ebene. Mathematiker schreiben Ausdrücke wie z.B. „$2x=6\implies x=3$“ oder „$V/\ker(f)\cong \im(f)$“ aufs Papier und verfallen über manche davon in tiefes Grübeln oder freuen sich ein Loch in den Bauch, nachdem sie irgendeinen wohl sehr bedeutsamen Ausdruck niedergeschrieben haben. Einem Außerirdischen, dessen unsagbar fremde Gedankenwelt völlig inkompatibel mit der menschlichen Weise, Mathematik zu betreiben, ist, wäre einzig diese Ebene unserer Mathematik zugänglich. Auch eine Rechenmaschine kann diese Ebene erfassen, selbst wenn wir ihr die Fähigkeit, mathematisch zu „denken“, absprechen. Für die philosophische Strömung des \emph{mathematischen Formalismus} ist dies die einzige Ebene, auf der Mathematik „existiert“.

    \item Die Vorstellungsebene. Ich habe gewisse Vorstellungen von Zahlen und geometrischen Figuren und sehe ein mathematisches Theorem, wie z.B. dass jede Gerade, die durch den Mittelpunkt eines Kreises verläuft, den Kreis in genau zwei Punkten schneidet, als eine Art „Wahrheit“ über diese Vorstellungen an. Diese Ebene ist rein subjektiv und ist, aus einer radikal solipsistischen Perspektive, nur mir selbst zugänglich. Die Strömung des \emph{mathematischen Platonismus} vertritt die Ansicht, dass die gedanklichen Vorstellungen in gewisser Weise objektiv sind, also dass die „Idee des Kreises“ auch unabhängig von meiner Gedankenwelt irgendwie „existiert“ und mathematische Theoreme gewissermaßen „ewige Wahrheiten“ über diese Ideen sind.
\end{enumerate}
Von gedanklichen Vorstellungen lassen sich sprachliche Abbilder anfertigen. So können wir Zahlen im Dezimalsystem aufschreiben und Zeichnungen von Geraden und Kreisen anfertigen. Die Wechselwirkung von Gedankenobjekten und ihren sprachlichen Bildern erweist sich fruchtbar sowohl für das Auffinden als auch das Überprüfen mathematischer „Wahrheiten“. So lernen wir bereits in der Grundschule eine Methode der schriftlichen Addition, die es uns ermöglicht, dreistellige Zahlen aufeinander zu addieren, von denen wir keine visuelle Vorstellung mehr haben. Umgekehrt wird ein Mathematiker, der ein geometrisches Theorem liest, dieses zuallerersteinmal mit seiner eigenen Gedankenwelt abgleichen.

In der späten Schulmathematik und in der Universitätsmathematik tritt nun eine dritte Ebene hinzu:
\begin{enumerate}
    \item[3.] Die formalisierte Ebene. Um meine Vorstellung präzisem mathematischen Schlussfolgern zugänglich, und damit zu einem \emph{mathematischen Objekt} zu machen, \textbf{formalisiere} ich sie. Dies kann auf zweierlei Arten geschehen:
    \begin{enumerate}
        \item(Bereitstellen eines Modells) Ich beschreibe in etablierten mathematischen Begriffen, was mein Objekt „ist“. So könnte man eine natürliche Zahl beispielsweise definieren als eine reelle Zahl, die sich ausgehend von der Null durch wiederholte Addition von Einsen erreichen lässt. Dies setzt voraus, dass bereits bekannt ist, was eine „reelle Zahl“ sein soll. Im Modell lassen sich nun Sätze über natürliche Zahlen beweisen, zum Beispiel, dass das Produkt zweier natürlicher Zahlen wieder eine natürliche Zahl ist und das Distributivgesetz erfüllt. Objekte, die im ersten Semester typischerweise per Modell eingeführt werden, sind: Komplexe Zahlen, Polynomringe, Faktorräume von Vektorräumen.

        Der Zugang über ein Modell ist in dem Sinne \textbf{analytisch}, dass bereits alle wahren und falschen Aussagen über mein Objekt durch das Modell vorgegeben sind. Eine Herleitung solcher Aussagen geschieht in demjenigen formalen Rahmen, der auch bereits das Modell bereitstellt. Werden etwa die komplexen Zahlen über ein konkretes Modell definiert, so sind nach und nach die herkömmlichen Rechenoperationen zu konstruieren und die für sie geltenden Rechenregeln herzuleiten. Der Vorteil der analytischen Herangehensweise besteht in einer gewissen „onthologischen Grundierung“. Indem von vornherein gesagt wird, was eine komplexe Zahl \emph{ist}, fällt es uns vielleicht einfacher, sich mit deren Existenz abzufinden, als den Generationen von Mathematikern zwischen dem 16. und 19. Jahrhundert, denen die „imaginären Zahlen“ als Mysterium erschienen.

        \item(Die axiomatische Methode) Anstatt zu sagen, was mein Objekt \emph{ist}, gebe ich lediglich an, wie es sich \emph{verhält}, d.h. ich gebe eine Reihe von Aussagen, sogenannte \textbf{Axiome}, an, die das Objekt meiner gedanklichen Vorstellung zufolge erfüllen soll. Axiomatische Definitionen der natürlichen Zahlen liefern zum Beispiel die berühmten Peano-Axiome oder die "natural numbers objects" aus der Topostheorie. Die axiomatische Methode bringt den Vorteil mit sich, dass man, ohne zuerst einen mathematischen Rahmen zur Bereitstellung eines Modells formulieren zu müssen, praktisch „aus dem Nichts“ damit beginnen kann, Mathematik zu betreiben, Aussagen herzuleiten, weiterführende Begriffe definieren usw. Der Nachteil ist jedoch, dass die axiomatische Methode an sich keine „ontologische Substanz“ liefert. Es ist erst einmal nicht klar, ob Objekte, die die Axiome erfüllen, überhaupt existieren. Ja, selbst für natürliche Zahlen ist dies schon fraglich: wenn es nur etwa $10^{80}$ Atome im beobachtbaren Universum gibt, inwiefern „existieren“ Zahlen wie $10^{90}$ überhaupt noch und inwiefern ist die mathematische Beschäftigung mit solchen Zahlen sinnvoll?

        Ein bekannter Witz unter Mathematikern ist die \emph{Legende vom antimetrischen Raum} (siehe \cref{???}), die von einem Doktoranden handelt, der seine Doktorarbeit über eine Klasse von „Räumen“ verfasst hat, die alle möglichen wünschenswerten Eigenschaften besitzen. Bei seiner Vertreidigungsrede fragt ihn schließlich einer der Prüfer nach einem Beispiel für einen solchen „antimetrischen Raum“. Der Doktorand gerät in Erklärungsnot und schließlich wird klar, dass es gar keine nichttrivialen Räume gibt, die seinem Antimetrik-Axiom genügen, die ganze Doktorarbeit also von einer Klasse völlig nutzloser Räume handelt.

        Objekte, die im ersten Semester typischerweise mittels Axiomen eingeführt werden, sind: Reelle Zahlen, die Determinante, \TODO
        \end{enumerate}
    In der Praxis verschwimmen beide Methoden, da die Modelle auf Begriffen beruhen, die wiederum axiomatisch eingeführt wurden, andererseits die Axiome sich Begriffe zunutze machen, die über ein Modell definiert sind.
\end{enumerate}
    \TODO
\end{comment}

...
\begin{de}[Relation] \label{def:relation} \index{Relation}
...
\begin{bsp} \quad \index{Teiler} \index{Vielfaches}
    \begin{enumerate}
        ...
        \begin{align*}
            n \mid m \qquad& :\Leftrightarrow\qquad \exists k\in \Z:\ k\cdot n=m && n,m\in \Z \\
            (\text{lies: „$n$ teilt $m$”})
        \end{align*}
        Dadurch wird eine Relation auf $\Z$ definiert, die \textbf{Teilbarkeitsrelation}. Sind $n,m\in \Z$ mit $n\mid m$, so heißen $n$ ein \textbf{Teiler} von $m$ und $m$ ein \textbf{Vielfaches} von $n$.
...
\begin{de}[feinere/gröbere Relation] \index{feinere Relation} \index{groebere Relation@gröbere Relation}
...
\begin{bsp} \quad
    \begin{enumerate}
        ...
        \item Seien $n,m\in \Z$. Die Aussage, dass $n$ kein Teiler von $m$ ist, wird notiert durch $n\nmid m$.
    \end{enumerate}
\end{bsp}


\begin{de}[Umkehrrelation] \label{def:umkehrrel} \index{Umkehrrelation}
...
\begin{de}[Eingeschränkte Relation] \index{Einschränkung einer Relation}
...
\begin{de}[Einige Eigenschaften von Relationen] \index{reflexive Relation} \index{transitive Relation} \index{symmetrische Relation} \index{antisymmetrische Relation}
...
\begin{de}[Ordnungsrelationen] \label{def:ordnungsrelation} \index{Ordnungsrelation} \index{Halbordnung} \index{partielle Ordnung} \index{geordnete Menge}
...
\begin{de}[Hasse-Diagramm] \label{def:hasse} \index{Hasse-Diagramm}
...
\begin{de}[Vergleichbarkeit] \label{def:totalordnung} \index{Totalordnung} \index{vergleichbar (Ordnungstheorie)}
...
\begin{de}[Intervalle in $\R$] \label{def:intervall} \index{Intervall}
...
\begin{de} \label{def:kleinstes} \index{groesstes Element@größtes Element} \index{Maximum} \index{kleinstes Element} \index{Minimum} \index{maximales Element} \index{minimales Element}
...
\begin{de}[Schranken] \label{def:schranken} \index{Schranke (Ordnungstheorie)} \index{beschränkt (Ordnungstheorie)}
...
\begin{de}[Supremum/Infimum] \index{Supremum} \index{Infimum}
...
\begin{de}[* Vertretersystem] \index{Vertretersystem} \index{Repräsentantensystem}
...
\begin{de}[Partition] \index{Partition}

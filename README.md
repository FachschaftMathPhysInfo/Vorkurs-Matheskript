...
Der gesamte LaTeX-Code erstreckt sich über:
    ...
    9. Die Shell-Datei "make.sh", die eine Prozedur enthält, um das Skript in einem Rutsch in den finalen Zustand zu kompilieren. Die Kompilation erfolgt mit LuaLaTex und benötigt relativ lange (auf meinem Rechner drei Minuten). LuaLaTeX wird benötigt, um die otf-Schriften zu laden und weitere Optimierungen vorzunehmen. Während der Arbeit am Skript ist das nicht nötig, hier genügt in der Regel einmaliges Kompilieren mit pdflatex (dauert bei mir etwa 10 Sekunden).
...
Erklärungen zur Dokumentstruktur:
    ...
    (amsthm-Umgebungen) Die Handhabung der amsthm-Umgebungen geschieht über das Paket thmtools. Die Definition der Umgebungen erfolgt in der "theoremstyles.tex" in zwei Phasen:
        ...
        2. Die eigentlichen Umgebungen mitsamt werden per \declaretheorem definiert. Beispielsweise sind \begin{bem} und \begin{vorschau} zwei verschiedene Umgebungen, teilen sich aber denselben theoremstyle "stybem".
    ...
    Der Grund dafür, dass nicht auch so etwas wie "satz:" oder "bem:" bei Sätzen und Bemerkungen verwendet wird, liegt darin, dass die Rollen von Bemerkungen und Sätzen stärker austauschbar sind, d.h. im Schreibprozess wird etwas, das vorher ein Satz war, plötzlich zum Axiom erhoben oder zur Bemerkung degradiert. Bedenkt, dass jede Änderung der labels umständlich ist, weil dann ja auch alle über den Code verteilten \cref{}-Instanzen geändert werden müssen.
    ...
    (Druckbarkeit) Das Skript ist grundsätzlich so geschrieben, dass es sich in doppelseitiger Buchansicht lesen lässt. Dies wird deutlich beispielsweise daran, dass die Kopfzeilen alternieren und das Skript stets auf einer geraden Seite mit dem griechischen Alphabet endet (sodass sich das griechische Alphabet im ausgedruckten Skript schnell auf der Rückseite nachschlagen lässt).

    (shorthands für die Listen) Denkt daran, dass euch das Paket "enumitem" komfortabel Listen nummerieren lässt! Im classfile werden mittels "\AtBeginEnvironment" Standard-Listentypen für einzelne amsthm-Umgebungen festgelegt. Wenn ihr beispielsweise in einer \begin{satz}-Umgebung eine enumerate-Liste startet, ist diese automatisch mit "a), b), c), ..." nummeriert. In einer \begin{bsp}-Umgebung dagegen mit "(1), (2), (3), ...". Bitte greift im Text auch auf diese Standard-Listen zurück, damit sich das globale Design leicht verändern lässt. Solltet ihr bspw. irgendwann beschließen, dass Beispiel standardmäßig durch "(a), (b), (c), ..." nummeriert sein sollten,  braucht ihr nur eine Zeile im classfile ändern und müsst nicht jede enumerate-Liste im ganzen Dokument abändern.

    (Verwendete Hacks, die das Verhalten von Standardbefehlen überschreiben)

        A) Die Standard-proof-Umgebung ist umdefiniert auf eine Beweis-Umgebung, die in der theoremstyles.tex definiert wird. In Beweisen wird automatisch auf den KOMA-Stil "parindent=half-" umgeschaltet, d.h. die Absätze bekommen größeren Abstand und dafür keine indents

        B) Die KOMA-Listenumgebung \begin{labeling} wurde umdefiniert. Sie nimmt nun elle enumitem-Parameter entgegen und berechnet automatisch das längste label und passt daran die Listeneinrückung an. Es muss und kann daher nicht mehr wie üblich das längste Label separat eingegeben werden. Für die korrekte Formatierung sind zwei Compilerdurchläufe nötig.

        C) Die Makros \frontmatter, \mainmatter, \appendix und \backmatter wurden im classfile umdefiniert. Dies betrifft ausschließlich kosmetische Einstellung wie, die Gestaltung des Inhaltsverzeichnisses, das Handling der pdf-bookmarks und der pagestyles, sowie die Nummerierung der theorems. Ebenso wurde \tableofcontents im classfile umdefiniert, um ein pdf-bookmark zu platzieren.
